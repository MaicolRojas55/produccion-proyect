...
export default function CalendarPage() {
  ...
  const visibleEvents = useMemo(() => {
    ...
    if (isStaffRole(user.role)) return dayEvents
    return dayEvents.filter(
      (e) => e.audience === 'todos' || e.audience === 'registrados'
    )
  ...
  const toggleAttend = (eventId: string) => {
    ...
    const updated = allEvents.map((e) => {
      ...
      return {
        ...e,
        attendees: has
          ? e.attendees.filter((x) => x !== user.id)
          : [...e.attendees, user.id]
      }
    })
    saveEvents(updated)
          ...
          <Card className="lg:col-span-7 p-4">
            ...
            <div className="mt-4 grid gap-3">
              {visibleEvents.length === 0 ? (
                ...
                visibleEvents.map((e) => {
                  const attending =
                    user.role === 'usuario_registrado' &&
                    e.attendees.includes(user.id)
                  const audienceLabel =
                    e.audience === 'todos'
                      ? 'Todos'
                      : e.audience === 'registrados'
                        ? 'Registrados'
                        : 'Staff'
                  return (
                    <div
                    ...
                    >
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          ...
                          <div className="text-sm text-muted-foreground">
                            {e.startTime} – {e.endTime} · Audiencia:{' '}
                            {audienceLabel}
                          </div>
                        </div>
